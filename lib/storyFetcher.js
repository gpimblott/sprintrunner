...
internals.getStoriesWithLabel = function (res, callback, label) {
  //Get the list of stories with the label
  var options = {
    url: 'https://www.pivotaltracker.com/services/v5/projects/' + res.app.get('pivotalProjectId') + '/stories?date_format=millis&with_label=' + label,
    headers: {
      'X-TrackerToken': res.app.get('pivotalApiKey')
    }
  };
...
storyFetcher.getLabelSummary = function (res, label) {
  async.parallel([
      function (callback) {
        internals.getStoriesWithLabel(res, callback, label);
      },
      function (callback) {
